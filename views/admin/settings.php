			...
			<table class="form-table">
				<tbody>
					...
					<tr valign="top">
						...
						<td>
							<select class="widefat" id="managewp_security_users" name="wp_user_security_users[]" multiple="multiple">
								<?php $users = get_users(); ?>
								<?php if (!empty(get_option('wp_user_security_users'))): ?>
									<?php foreach( $users as $user ): ?>
										<?php echo '<option value="'.$user->ID.'" '.(in_array($user->ID, get_option('wp_user_security_users')) ? 'selected="selected"' : '').'>'.$user->data->display_name.'</option>'; ?>
									<?php endforeach; ?>
								<?php else: ?>
									<?php foreach( $users as $user ): ?>
										<?php echo '<option value="'.$user->ID.'" '.'>'.$user->data->display_name.'</option>'; ?>
									<?php endforeach; ?>
								<?php endif; ?>
							</select>
						</td>
